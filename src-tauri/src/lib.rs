...
use crate::chromium::vimium::{
    chrome_clear_page_hints, chrome_interact_with_element, chrome_show_page_hints,
};
...
fn parse_command(input: &str) -> Result<ParsedCommand, String> {
    ...
    if parts.len() < 2 {
        return Err(
            "Command must have at least action and target (e.g., 'Open Chrome')".to_string(),
        );
    }
...
fn open_application(app_name: &str) -> CommandResponse {
    let result = match app_name {
        "chrome" | "google chrome" => {
            ...
            let cmd_result = Command::new("cmd").args(["/C", "start", "chrome"]).output();

            #[cfg(target_os = "macos")]
            let cmd_result = Command::new("open").args(["-a", "Google Chrome"]).output();
        ...
        "firefox" => {
            ...
            #[cfg(target_os = "macos")]
            let cmd_result = Command::new("open").args(["-a", "Firefox"]).output();

            #[cfg(target_os = "linux")]
            let cmd_result = Command::new("firefox").output();

            cmd_result
        }
        "notepad" => {
            #[cfg(target_os = "windows")]
            let cmd_result = Command::new("notepad").output();

            #[cfg(target_os = "macos")]
            let cmd_result = Command::new("open").args(["-a", "TextEdit"]).output();
        ...
        "file manager" | "explorer" | "finder" => {
            #[cfg(target_os = "windows")]
            let cmd_result = Command::new("explorer").output();

            #[cfg(target_os = "macos")]
            let cmd_result = Command::new("open").args(["-a", "Finder"]).output();
